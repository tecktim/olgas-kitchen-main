(function(){const n=document.createElement("link").relList;if(n&&n.supports&&n.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))a(e);new MutationObserver(e=>{for(const r of e)if(r.type==="childList")for(const i of r.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&a(i)}).observe(document,{childList:!0,subtree:!0});function o(e){const r={};return e.integrity&&(r.integrity=e.integrity),e.referrerPolicy&&(r.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?r.credentials="include":e.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function a(e){if(e.ep)return;e.ep=!0;const r=o(e);fetch(e.href,r)}})();async function f(){const t=await fetch("/data.json");if(!t.ok)throw new Error(`Failed to load data: ${t.status}`);return t.json()}function y(t){document.title=t;const n=document.querySelector("header h1");console.log("base URL:","/"),n.innerHTML=`<img src="/logo.svg" alt="Logo" class="logo"> ${t}`}function v(t){const n=document.querySelector("nav ul");n.innerHTML="",t.forEach(o=>{const a=document.createElement("li"),e=document.createElement("a");e.href=o.href,e.textContent=o.text,a.appendChild(e),n.appendChild(a)})}function b(t){const n=document.getElementById("home");n.innerHTML=`
    <h2>${t.heading}</h2>
    <p>${t.text}</p>
    <a href="${t.button.href}" class="btn">${t.button.text}</a>
  `}const g="1.0.2";console.log("Version:",g);function $(t){const n=document.querySelector(".products");n.innerHTML="",t.forEach(o=>{const a=document.createElement("div");a.className="product",a.dataset.images=JSON.stringify(o.images),a.innerHTML=`
    ...
    `,n.appendChild(a);const e=o.images,r=a.querySelector("img.product-image"),i=a.querySelector("div.product-overlay");let s=0;const c=a.querySelector(".product-image-wrapper");if(e.length>1){let u=function(){const{src:l,overlayText:h}=e[s];r.src=`/${l}?v=${g}`,i.textContent=h};var m=u;r.style.opacity="1",r.style.transition="opacity 0.3s ease-in-out",i.style.opacity="1",i.style.transition="opacity 0.3s ease-in-out",c.addEventListener("click",l=>{l.stopPropagation();const h=c.getBoundingClientRect();l.clientX-h.left<h.width/2?s=(s-1+e.length)%e.length:s=(s+1)%e.length,r.style.opacity="0",i.style.opacity="0",setTimeout(()=>{u(),r.style.opacity="1",i.style.opacity="1"},200)});let d=null;c.addEventListener("touchstart",l=>{d=l.changedTouches[0].clientX}),c.addEventListener("touchend",l=>{if(d===null)return;const p=l.changedTouches[0].clientX-d;Math.abs(p)>30&&(p>0?s=(s-1+e.length)%e.length:s=(s+1)%e.length,r.style.opacity="0",i.style.opacity="0",setTimeout(()=>{u(),r.style.opacity="1",i.style.opacity="1"},200)),d=null}),c.addEventListener("pointerdown",l=>{d=l.clientX}),c.addEventListener("pointerup",l=>{if(d===null)return;const p=l.clientX-d;Math.abs(p)>30&&(p>0?s=(s-1+e.length)%e.length:s=(s+1)%e.length,r.style.opacity="0",i.style.opacity="0",setTimeout(()=>{u(),r.style.opacity="1",i.style.opacity="1"},200)),d=null})}else c.classList.add("single-image")})}function L(t){const n=document.getElementById("bestellen");n.innerHTML=`
    <h2>${t.heading}</h2>
    <div class="process">
