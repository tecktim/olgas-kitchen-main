(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))u(t);new MutationObserver(t=>{for(const e of t)if(e.type==="childList")for(const i of e.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&u(i)}).observe(document,{childList:!0,subtree:!0});function r(t){const e={};return t.integrity&&(e.integrity=t.integrity),t.referrerPolicy&&(e.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?e.credentials="include":t.crossOrigin==="anonymous"?e.credentials="omit":e.credentials="same-origin",e}function u(t){if(t.ep)return;t.ep=!0;const e=r(t);fetch(t.href,e)}})();async function L(){const n=await fetch("/data.json");if(!n.ok)throw new Error(`Failed to load data: ${n.status}`);return n.json()}function $(n){document.title=n;const a=document.querySelector("header h1");console.log("base URL:","/"),a.innerHTML=`<img src="/logo.svg" alt="Logo" class="logo" onclick="window.location.href='https://olgas-kitchen.de'" style="cursor:pointer;"> <span onclick="window.location.href='https://olgas-kitchen.de'" style="cursor:pointer;">${n}</span>`}function E(n,a=[]){const r=document.querySelector("nav ul");if(r.innerHTML="",n.forEach(e=>{const i=document.createElement("li"),o=document.createElement("a");o.href=e.href,o.textContent=e.text,i.appendChild(o),r.appendChild(i)}),a.length){let v=function(c){h.innerHTML="";const m=document.createElement("img");m.src=`/${c.icon}`,m.alt=c.code,m.width=32,m.height=24,h.appendChild(m);const l=document.createElement("span");l.textContent=" "+c.code.toUpperCase(),l.style.fontWeight="bold",l.style.marginLeft="0.5em",h.appendChild(l)},g=function(){f.innerHTML="",a.filter(c=>c.code!==s.code).forEach(c=>{const m=document.createElement("li"),l=document.createElement("a");l.href=c.href,l.style.display="flex",l.style.alignItems="center";const p=document.createElement("img");p.src=`/${c.icon}`,p.alt=c.code,p.width=32,p.height=24,l.appendChild(p);const d=document.createElement("span");d.textContent=" "+c.code.toUpperCase(),d.style.fontWeight="bold",d.style.marginLeft="0.5em",l.appendChild(d),l.addEventListener("click",y=>{y.preventDefault(),s=c,v(s),g(),f.classList.add("hidden"),window.location.search!==c.href&&(window.location.search=c.href)}),m.appendChild(l),f.appendChild(m)})};var u=v,t=g;const e=document.createElement("li");e.classList.add("language-selector");const o=new URLSearchParams(window.location.search).get("lang");let s=a.find(c=>c.code===(o?o.toLowerCase():"de"))||a[0];const h=document.createElement("button");h.type="button",h.classList.add("lang-button"),v(s),e.appendChild(h);const f=document.createElement("ul");f.classList.add("lang-dropdown","hidden"),g(),e.appendChild(f),r.appendChild(e),h.addEventListener("click",c=>{c.stopPropagation(),f.classList.toggle("hidden")}),window._langDropdownListener&&document.removeEventListener("click",window._langDropdownListener),window._langDropdownListener=function(c){e.contains(c.target)||f.classList.add("hidden")},document.addEventListener("click",window._langDropdownListener)}}function x(n){const a=document.getElementById("home");a.innerHTML=`
    <h2>${n.heading}</h2>
    <p>${n.text}</p>
    <a href="${n.button.href}" class="btn">${n.button.text}</a>
  `}const b="1.0.6";console.log("Version:",b);function S(n){const a=document.querySelector(".products");a.innerHTML="",n.forEach(r=>{const u=document.createElement("div");u.className="product",u.dataset.images=JSON.stringify(r.images),u.innerHTML=`
    ...
    `,a.appendChild(u);const t=r.images,e=u.querySelector("img.product-image"),i=u.querySelector("div.product-overlay");let o=0;const s=u.querySelector(".product-image-wrapper");if(t.length>1){let g=function(){const{src:d,overlayText:y}=t[o];e.src=`/${d}?v=${b}`,i.textContent=y},m=function(){clearInterval(c),c=setInterval(()=>{o=(o+1)%t.length,e.style.opacity="0",i.style.opacity="0",setTimeout(()=>{g(),e.style.opacity="1",i.style.opacity="1"},200)},4e3)},l=function(){clearInterval(c),setTimeout(m,8e3)};var h=g,v=m,f=l;e.style.opacity="1",e.style.transition="opacity 0.3s ease-in-out",i.style.opacity="1",i.style.transition="opacity 0.3s ease-in-out";let c;m(),s.addEventListener("click",d=>{d.stopPropagation();const y=s.getBoundingClientRect();d.clientX-y.left<y.width/2?o=(o-1+t.length)%t.length:o=(o+1)%t.length,e.style.opacity="0",i.style.opacity="0",setTimeout(()=>{g(),e.style.opacity="1",i.style.opacity="1"},200),l()});let p=null;s.addEventListener("touchstart",d=>{p=d.changedTouches[0].clientX}),s.addEventListener("touchend",d=>{if(p===null)return;const w=d.changedTouches[0].clientX-p;Math.abs(w)>30&&(w>0?o=(o-1+t.length)%t.length:o=(o+1)%t.length,e.style.opacity="0",i.style.opacity="0",setTimeout(()=>{g(),e.style.opacity="1",i.style.opacity="1"},200),l()),p=null}),s.addEventListener("pointerdown",d=>{p=d.clientX}),s.addEventListener("pointerup",d=>{if(p===null)return;const w=d.clientX-p;Math.abs(w)>30&&(w>0?o=(o-1+t.length)%t.length:o=(o+1)%t.length,e.style.opacity="0",i.style.opacity="0",setTimeout(()=>{g(),e.style.opacity="1",i.style.opacity="1"},200),l()),p=null})}else s.classList.add("single-image")})}function k(n){const a=document.getElementById("bestellen");a.innerHTML=`
    <h2>${n.heading}</h2>
    <div class="process">
